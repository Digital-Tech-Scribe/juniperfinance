...
const Insights: React.FC = () => {
  ...
  return (
    <section id="insights" className="py-12 lg:py-16 bg-background-secondary relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div 
          ref={reveal}
          className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 scroll-reveal"
        >
          <div>
          ...
          <Button variant="outline" className="border-border text-foreground-secondary hover:bg-background-tertiary hover:border-accent/50 self-start sm:self-auto magnetic-btn">
            View All Articles
        ...
        <div 
          ref={reveal}
          className="mb-8 scroll-reveal reveal-delay-200"
        >
          <GlassCard className="group overflow-hidden tilt-effect" hover>
            <div className="grid lg:grid-cols-2">
              {/* Image */}
              <div className="aspect-video lg:aspect-auto bg-background-secondary relative overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-20 h-20 bg-accent/10 rounded-full flex items-center justify-center border border-accent/20 transition-transform group-hover:scale-110 duration-500">
              ...
              <div className="p-8 lg:p-10 flex flex-col justify-center">
                ...
                <Link to={`/article/${featuredInsight.id}`}>
                  <Button className="bg-accent hover:bg-accent/90 text-white self-start shadow-lg hover:shadow-accent/40 transition-all magnetic-btn">
                    Read Article
                    <ArrowRight className="w-4 h-4 ml-2" />
                  </Button>
                </Link>
          ...
          {insights.slice(1).map((article, index) => {
            ...
            return (
              <article
                key={article.id}
                ref={reveal}
                className={`group scroll-reveal reveal-delay-${(index + 3) * 100}`}
              >
                <Link to={`/article/${article.id}`} className="block h-full">
                  <GlassCard className="overflow-hidden h-full tilt-effect" hover>
                    {/* Image */}
